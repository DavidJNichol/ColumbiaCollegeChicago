...
namespace VirtualPetAngelo
{
    public class VirtualGame
    {
        ...
        {
            Console.Clear();
            Console.WriteLine($"{player.PlayerName} has ${player.PlayerWallet}.");
            Console.WriteLine("Choose you Pet :");
            bool choosed = true;
            
            foreach(Pet pet in pets)
            {
                Console.WriteLine($"{pets.IndexOf(pet)+1}.{pet.ToString().Replace("VirtualPetAngelo.", "")} Price: ${pet.PetPrice}");
            }
            ...
            while (choosed)
            {
                ...
                if (pets[index - 1].PetPrice > player.PlayerWallet)
                {
                    Console.WriteLine("You don't have enough money to buy this pet");
                    Console.WriteLine("Choose another Pet");
                    choosed = true;
                }
                else
                {
                    try
                    {
                        ...
                        choosed = false;
                        player.PlayerWallet = player.PlayerWallet - pets[index - 1].PetPrice;
                    }
                    catch (Exception ex)
                    {
                        throw ex;
